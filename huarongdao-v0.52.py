...
#        print("Map is",map)
        if(map==1):        
            ...
            logging.info(u"选择地图"+str(map)+u"初始状态如下")
            break
        elif(map==2):            
            #2、地图“齐头并前”
            #张曹曹赵
...
def handleOpen():
    ...
    while True:
        select=input('请输入需要的解法数量(0-9,0代表所有解法):')
        if(select in "0123456789"):
            answer_count=int(select)
            logging.info(u"选择"+select+"种解法")
            break
        else:
            print("\n wrong input, please choose again")
           
    begin_time = int(time.time())
    logging.info(u"开始时间"+str(begin_time))
...
#v0.5将每个新的node更新到dict，更好的做法是放到检查与前面open或close节点是否等价的检查那做。
                  handleDict(tmp[y])
...
#原有判断还不够精细，需要增加同等项6/7/8/9四个小卒组合位置相同的，也认为是同一局型。
                        if(check_match(tmp[y][0],opened[jj][0])==True):
                            flag=True #设置node已存在的标记
#后面三行是使用新节点的距离关系更新到他等价的节点上
                            tmpcheck = copy.deepcopy(tmp[y])
                            tmpcheck[0]=copy.deepcopy(opened[jj][0])
                            handleDict(tmpcheck)
...
#后面三行是使用新节点的距离关系更新到他等价的节点上
                            tmpcheck = copy.deepcopy(tmp[y])
                            tmpcheck[0]=copy.deepcopy(closed[kk][0])
                            handleDict(tmpcheck)
...
#以下定期打印进度情况
          ...
          if((len(closed)+len(opened))%100>=0 and (len(closed)+len(opened))%100<1):
              print('已用时:',int(current_time-begin_time),'s, 预计剩余:',int(int(current_time-begin_time)*len(closed)/len(opened)),'s')
...
#'''
    end_time = int(time.time())
    print('已找到答案，共用时:',end_time-begin_time)
    if(len(oklist)==0):
        print('No answer!')
    else:
...
#            print('last:',rolex)
            ...
            logging.info(u"选择返回上一步,因没有上一步,选择失败")
